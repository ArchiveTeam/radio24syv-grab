...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if allowed(url, nil) then
    ...
    if string.match(url, "^https?://api%.radio24syv%.dk/v2/podcasts/[0-9]+$") then
      ...
      check("http://arkiv.radio24syv.dk/attachment" .. data["audioInfo"]["url"] .. "?source=websitedownload")
      --local center_a, center_b = string.match(data["image"]["center"], "^([0-9%.]+)%,([0-9%.]+)$")
      --check(data["image"]["src"] .. "?width=931&height=499&mode=crop&center=" .. center_b .. "," .. center_a)
      return urls
    end
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  if string.match(url["url"], "^https?://api%.radio24syv%.dk/v2/podcasts/[0-9]+$") then
